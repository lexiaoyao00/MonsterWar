...
private:
    ...
    entt::dispatcher* dispatcher_;                                        ///< @brief 用于触发动作回调的 entt::dispatcher 指针
...
public:
     ...
     * @brief 注册一个动作的回调函数
     *
     * @param action_name 动作名称哈希
     * @param action_state 动作状态，默认按下状态
     * @return entt::sink<entt::sigh<void()>> 一个 sink 对象，用于注册回调函数
     */
    ...
    void quit();                                     ///< @brief 退出程序

    // 动作状态检查，提供不同的使用选择（轮询方式）
