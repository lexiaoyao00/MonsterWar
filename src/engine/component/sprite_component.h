...
/**
 * @brief 管理 GameObject 的视觉表示，通过持有一个 Image 对象。
 *
 * 协调 Image 数据和渲染逻辑，并与 TransformComponent 交互。
 */
...
private:
    ...
    engine::render::Image image_;                                         ///< @brief 精灵对象
...
public:
    ...
    // Getters
    const engine::render::Image& getSprite() const { return image_; }         ///< @brief 获取精灵对象
    std::string_view getTextureId() const { return image_.getTextureId(); }  ///< @brief 获取纹理ID
    bool isFlipped() const { return image_.isFlipped(); }                      ///< @brief 获取是否翻转
    ...
    void setFlipped(bool flipped) { image_.setFlipped(flipped); }                                             ///< @brief 设置是否翻转
...
private:
    void updateSpriteSize();        ///< @brief 辅助函数，根据 image_ 的 source_rect_ 更新 sprite_size_
