...
SpriteComponent::SpriteComponent(
    ...
    bool is_flipped
) : resource_manager_(&resource_manager), image_(texture_id, std::move(source_rect_opt), is_flipped),
    alignment_(alignment)
{
    ...
    spdlog::trace("创建 SpriteComponent，纹理ID: {}", image_.getTextureId());
}
...
SpriteComponent::SpriteComponent(engine::render::Image&& sprite, engine::resource::ResourceManager& resource_manager, engine::utils::Alignment alignment)
    : resource_manager_(&resource_manager), image_(std::move(sprite)), alignment_(alignment)
{
    if (!resource_manager_) {
        spdlog::critical("创建 SpriteComponent 时 ResourceManager 为空！，此组件将无效。");
        // 不要在游戏主循环中使用 try...catch / throw，会极大影响性能
...
void SpriteComponent::render(engine::core::Context& context) {
    ...
    // 执行绘制
    context.getRenderer().drawSprite(context.getCamera(), image_, pos, scale, rotation_degrees);
}

void SpriteComponent::setSpriteById(std::string_view texture_id, std::optional<SDL_FRect> source_rect_opt) {
    image_.setTextureId(texture_id);
    image_.setSourceRect(std::move(source_rect_opt));
...
void SpriteComponent::updateSpriteSize() {
    ...
    if (image_.getSourceRect().has_value()) {
        const auto& src_rect = image_.getSourceRect().value();
        sprite_size_ = {src_rect.w, src_rect.h};
    } else {
        sprite_size_ = resource_manager_->getTextureSize(image_.getTextureId());
    }
}
