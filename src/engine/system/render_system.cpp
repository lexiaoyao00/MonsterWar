...
void RenderSystem::update(entt::registry &registry, render::Renderer &renderer, render::Camera &camera)
{
    ...
    for (auto entity : view) {
        ...
        auto position = transform.position_ + sprite.offset_;   // 位置 = 变换组件的位置 + 精灵组件的偏移
        auto size = sprite.size_ * transform.scale_;   // 大小 = 精灵组件的大小 * 变换组件的缩放
