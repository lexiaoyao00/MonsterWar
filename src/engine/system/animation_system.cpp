...
void AnimationSystem::update(entt::registry &registry, float dt)
{
    ...
    for (auto entity : view) {
        ...
        auto &sprite_componet = view.get<component::SpriteComponent>(entity);
        ...
        // 检查是否需要切换到下一帧
        if (anim_component.current_time_ms_ >= current_frame.durationi_ms_) {
            anim_component.current_time_ms_ -= current_frame.durationi_ms_;
        ...
        sprite_componet.sprite_.stc_rect_ = next_frame.stc_rect_;
    }
}
