...
struct SpriteBlueprint {
    ...
    bool face_right_{true};         // 角色图片默认chaos右向
};

/**
 * @brief 单一动画蓝图，多个蓝图联合用于创建动画组件
 *
 */
...
struct EnemyBlueprint {
    bool ranged_{false};        // 是否是远程攻击
    float speed_{};
};
/**
 * @brief 显示信息蓝图，用于查找对应职业的名称和描述
 *
 */
...
struct BuffBlueprint {
    ...
    float cost_regen_{0.0f}; // 每秒回复消耗
};

/// @brief 玩家技能蓝图，用于创建技能组件
